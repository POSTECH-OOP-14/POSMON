...
public class BattleButtonManage : MonoBehaviour
{
        ...
        while (true)
        {
            ...
            else if (Battle == BattleButtonState.ExchangeState)
            {
                ...
                if (i != 0)
                {
                    ...
                    {
                        printTextbox("그 학생은 싸우기엔 너무 지쳐있다." + "\n(z나 enter를 입력)").MoveNext();
                        i = 0;
                    }
                    else
                    {
            ...
            else if (Battle == BattleButtonState.OneOutState)
            {
                ...
                if (i != 0)
                {
                    ...
                    //선택한 버튼이 기절한 학생이라면
                    else if (MineStudentList[i - 1].retStuStatus() == status.faint || MineStudentList[i - 1].getHP() < 0.5)
                    {
                        printTextbox("그 학생은 이미 지쳐 있다!" + "\n(z나 enter를 입력)");
                        i = 0;
                    }
                    else
                    {
                        ...
                        printTextbox(CurrentMine.retStudentName() + "을 내보냈다!\n" + "(z나 enter를 입력)");
                        Battle = BattleButtonState.DefaultState;
                    }
                }
            }
                ...
                if (whoFirst == 0)  //player go first
                {
                    ...
                    {                          //write how to change character.
                        if (toChangeP != -1)
                        {
                            ...
                            printTextbox("교수님은" + MineStudentList[toChangeP].retStudentName() + "을 내보냈다!");
                        }
                        else
                        {
                            printTextbox("유효하지 않은 상태입니다.").MoveNext();
                        }
                    }
                    else if (myMove == 1)
                    {
                    ...
                    else if (battleStartMove != 0)
                    {
                        ...
                        printTextbox(CurrentMine.retStudentName() + "은 움직일 수 없다." + "\n" + "(z나 enter를 입력)").MoveNext();
                    }
                    else if (myMove == 0) // 내 행동이 공격일 때
                    {
                        alived = CurrentEnemy.getDamage(damage);
                        Debug.Log("아군이 먼저 공격을 시작했다.");
                        printTextbox(CurrentMine.retStudentName() + "은 " + damage.ToString() + "만큼의 데미지를 주었다.\n" + "(z나 enter를 입력)").MoveNext();

                        if (alived == 1) //if enemy student fainted.
                        {
                            printTextbox("상대 " + CurrentEnemy.retStudentName() + "은 " + "쓰러졌다" + "\n(z나 enter를 입력)").MoveNext();
                            ...
                            printTextbox(CurrentMine.retStudentName() + "은 " + exp.ToString() + "만큼의 경험치를 얻었다." + "\n(z나 enter를 입력)").MoveNext();
                    ...
                    {  //enemy attacks.
                        ...
                        {
                            printTextbox("상대는 움직일 수 없었다" + "\n(z나 enter를 입력)");
                            //여기서 상태이상으로 행동이 종료되었을 때 메세지를 출력하면 된다.,
                        }
                        else
                        {
                            alived = CurrentMine.getDamage(myToOppoDamage);
                            printTextbox("상대는 " + myToOppoDamage.ToString() + "만큼의 피해를 주었다." + "\n(z나 enter를 입력)").MoveNext();
                        }

                        if (alived == 1)    // if our student dead.
                        {
                            CurrentMine.giveAStatus(status.faint);
                            printTextbox(CurrentMine.retStudentName() + "은 쓰러졌다." + "\n(z나 enter를 입력)").MoveNext();
                        ...
                        //상대와 나의 행동이 끝나고 전투가 끝나지 않았다면, 전투 상태는 다시 기본 상태로 돌아간다.
                        if (Battle != BattleButtonState.RunState || Battle != BattleButtonState.ExchangeState)
                            Battle = BattleButtonState.DefaultState;
                    }
                }
                else  // oppponent go first
                {
                    ...
                    if (Battle == BattleButtonState.NextState)
                    {
                        ...
                        if (myMove == 2)    //if i change character.
                        {
                            ...
                            CurrentMine = MineStudentList[toChangeP];
                            if (toChangeP != -1)
                                printTextbox("교수님은" + MineStudentList[toChangeP].retStudentName() + "을 내보냈다!").MoveNext();
                            else
                                printTextbox("유효하지 않은 상태입니다.");
                        ...
                        {
                            //여기서 상태이상으로 행동이 종료되었을 때 메세지를 출력하면 된다.,
                            printTextbox(CurrentMine.retStuIndex().ToString() + "번 학생은 움직일 수 없다." + "\n(z나 enter를 입력)");
                        }
                        else if (myMove == 0) // 내 행동이 공격일 때
                        {
                            alived = CurrentEnemy.getDamage(damage);
                            ...
                            if (alived == 1) //if enemy student fainted.
                            {
                                ...
                                printTextbox(CurrentEnemy.retStudentName() + "은 " + damage.ToString() + "쓰러졌다" + "\n(z나 enter를 입력)").MoveNext();

                                CurrentMine.setExp(CurrentEnemy.getExp()); //give exp;

                                printTextbox(CurrentEnemy.retStuIndex().ToString() + "학생은 " + CurrentEnemy.getExp().ToString() + "만큼의 경험치를 얻었다." + "\n(z나 enter를 입력)").MoveNext();
    ...
    IEnumerator printTextbox(string a)
    {
        int asdf = 0;
        bool aba = false;
        while (!aba)
        {
            GUI.Box(new Rect(1, Screen.height - 100, 600, 100), a);

            while(asdf == 0)
            {
                while (Input.GetKey(KeyCode.Z) || Input.GetKey(KeyCode.KeypadEnter) || Input.GetMouseButton(0))
                {
                    GUI.Box(new Rect(1, Screen.height - 100, 600, 100), a);
                    asdf = 1;
                    yield return null;
